...
import System.Directory
         ...
         , doesPathExist, removeFile, removeDirectory, copyFile )
...
installExes verbosity baseCtx buildCtx compiler clientInstallFlags = do
  ...
  let mkPkgBinDir = (</> "bin") .
      ...
      copydirUnknown =
        "copydir is not defined. Set it in your cabal config file "
        ++ "or use --copydir=<path>"
  ...
  copyBindir    <- fromFlagOrDefault (die' verbosity copydirUnknown)
